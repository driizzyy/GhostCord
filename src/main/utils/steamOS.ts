...
const layoutId = "3080264545"; // GhostCord Layout v2
...
export async function askToApplySteamLayout(win: BrowserWindow) {
    ...
    const { response } = await dialog.showMessageBox(win, {
        message: `${update ? "Update" : "Apply"} DriizzyyB Steam Input Layout?`,
        detail: `Would you like to ${update ? "Update" : "Apply"} DriizzyyB's recommended Steam Deck controller settings?
