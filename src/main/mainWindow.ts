...
import { GhostCordSettings, Settings, State } from "./settings";
import { createSplashWindow, updateSplashMessage } from "./splash";
import { downloadGhostCordFiles, ensureGhostCordFiles } from "./utils/ghostcordLoader";
...
const [addGhostCordSettingsListener, removeGhostCordSettingsListeners] = makeSettingsListenerHelpers(GhostCordSettings);
...
function initMenuBar(win: BrowserWindow) {
    ...
    const wantCtrlQ = !isWindows || GhostCordSettings.store.winCtrlQ;
...
function getDarwinOptions(): BrowserWindowConstructorOptions {
    ...
    const { macosTranslucency } = GhostCordSettings.store;
...
function initSettingsListeners(win: BrowserWindow) {
    ...
    addGhostCordSettingsListener("macosTranslucency", enabled => {
        if (enabled) {
...
const runGhostCordMain = once(() => require(join(GHOSTCORD_FILES_DIR, "ghostcordDesktopMain.js")));
