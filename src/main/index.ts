...
console.log("DriizzyyB" + app.getVersion());

// Make the GhostCord files use our DATA_DIR
...
function init() {
    ...
    app.whenReady().then(async () => {
        if (process.platform === "win32") app.setAppUserModelId("dev.ghostcord.ghostcord");
...
if (!app.requestSingleInstanceLock({ IS_DEV })) {
    if (IS_DEV) {
        console.log("DriizzyyB is already running. Quitting previous instance...");
        init();
    } else {
        console.log("DriizzyyB is already running. Quitting...");
        app.quit();
    }
} else {
    init();
